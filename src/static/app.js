document.addEventListener("DOMContentLoaded", () => {
              ...
              ${details.participants && details.participants.length > 0
                ? details.participants.map(p => `
                    <li class="participant-item">
                      ...
                      <button class="participant-delete" data-activity="${encodeURIComponent(name)}" data-email="${escapeHTML(p)}" aria-label="Remove ${escapeHTML(p)}">×</button>
                    </li>
                  `).join("")
  ...
  activitiesList.addEventListener("click", async (event) => {
    ...
    try {
      const response = await fetch(
        `/activities/${activityName}/participants?email=${encodeURIComponent(email)}`,
        {
          method: "DELETE",
        }
      );
